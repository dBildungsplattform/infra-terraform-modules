locals {
  ...
  # privacyidea_nicIndex = index(data.ionoscloud_servers.privacyidea_servers.servers[0].nics.*.lan, tonumber(var.lan_id))
  # privacyidea_prefix   = format("%s/24", data.ionoscloud_servers.privacyidea_servers.servers[0].nics[local.privacyidea_nicIndex].ips[0])
}
