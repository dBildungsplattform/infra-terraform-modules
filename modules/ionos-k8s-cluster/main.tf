...
resource "ionoscloud_k8s_node_pool" "nodepool_scaling" {
  ...
  public_ips     = each.value.create_public_ip_pools == true ? ionoscloud_ipblock.ippools[each.key].ips : (each.value.max_node_count > length(each.value.public_ips[each.value.nodepool_index]) ? [] : slice(each.value.public_ips[each.value.nodepool_index], 0, each.value.max_node_count))
...
resource "ionoscloud_k8s_node_pool" "nodepool_legacy" {
  ...
  ram_size       = each.value.ram_size
  storage_size   = each.value.storage_size
  public_ips     = each.value.create_public_ip_pools ? ionoscloud_ipblock.ippools[each.key].ips : each.value.public_ips == null ? [] : each.value.node_count > length(each.value.public_ips[each.value.nodepool_index]) ? [] : slice(each.value.public_ips[each.value.nodepool_index], 0, each.value.node_count) #slice(public_ips[0] = [""],0,2)
}
