...
resource "ionoscloud_k8s_node_pool" "nodepool_scaling" {
  ...
  public_ips     = each.value.create_public_ip_pools == true ? ionoscloud_ipblock.ippools[each.key].ips : slice(each.value.public_ips[each.value.nodepool_index], 0, each.value.max_node_count)
...
resource "ionoscloud_k8s_node_pool" "nodepool_legacy" {
  ...
  name              = each.value.purpose != "legacy" ? each.key : each.value.availability_zone == "ZONE_1" ? "${local.cluster_name}-zone1-nodepool-0":"${local.cluster_name}-zone2-nodepool-0"
  k8s_version       = ionoscloud_k8s_cluster.cluster.k8s_version
  allow_replace     = each.value.allow_node_pool_replacement
  # the lans are created as a dynamic block - they help to dynamically construct repeatable nested blocks
  # it iterates through the list of var.associated_lans and sets the appropriate lan id
  ...
  ram_size       = each.value.ram_size
  storage_size   = each.value.storage_size
  public_ips     = each.value.create_public_ip_pools ? ionoscloud_ipblock.ippools[each.key].ips : each.value.public_ips == null ? [] : slice(each.value.public_ips[each.value.nodepool_index], 0, each.value.node_count)
}
